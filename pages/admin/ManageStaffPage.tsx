...
const ManageStaffPage: React.FC = () => {
    ...
    const [formData, setFormData] = useState<StaffFormData>({ name: '', email: '', phone: '', address: '' });
    ...
    const handleOpenModal = (user: User | null) => {
        setEditingUser(user);
        setFormData(user ? { name: user.name, email: user.email, phone: user.phone || '', address: user.address || '' } : { name: '', email: '', password: '', phone: '', address: '' });
        setIsModalOpen(true);
    };
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-center border-b border-base-300 pb-4 mb-8 gap-4">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-serif font-bold text-neutral">Manage Staff</h1>
                </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {paginatedStaff.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="p-4 text-center text-neutral-800">No staff members found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {/* Add/Edit Modal */}
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingUser ? 'Edit Staff Member' : 'Add New Staff Member'}>
                <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto p-1">
                    <InputField label="Full Name" id="name" name="name" type="text" value={formData.name} onChange={handleFormChange} required autoFocus />
                    <InputField label="Email" id="email" name="email" type="email" value={formData.email} onChange={handleFormChange} required />
                    <InputField label="Phone Number" id="phone" name="phone" type="tel" value={formData.phone || ''} onChange={handleFormChange} placeholder="e.g. 123-456-7890" />
                    <InputField label="Address" id="address" name="address" type="text" value={formData.address || ''} onChange={handleFormChange} placeholder="e.g. 123 Main St, Anytown" />
                    <InputField label="Password" id="password" name="password" type="password" placeholder={editingUser ? 'Leave blank to keep current' : ''} onChange={handleFormChange} required={!editingUser} />
            ...
            <Modal isOpen={isDeleteModalOpen} onClose={handleCloseDeleteModal} title="Confirm Deletion">
                <div className="space-y-4">
                    <p>Are you sure you want to delete the staff member "<span className="font-bold">{userToDelete?.name}</span>"?</p>
