...
const DishCard: React.FC<{ dish: Dish }> = ({ dish }) => {
  ...
  const handleAddToOrder = () => {
    addToCart(dish, 1);
    showNotification(`${dish.name} added to order.`);
  };
    ...
    <div className="bg-base-100 rounded-xl overflow-hidden border border-base-300 shadow-sm hover:shadow-xl hover:border-gray-300 transition-all duration-300 transform hover:-translate-y-1 flex flex-col group">
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-serif font-bold text-xl text-neutral group-hover:text-primary transition-colors">
            {dish.name}
          </h3>
        ...
        <div className="flex items-center mb-3">
          ...
          <span className="text-neutral-800 text-sm ml-2">
            ({dish.reviews} reviews)
          </span>
        </div>
        <p className="text-neutral-800 text-base flex-grow mb-4">
          {dish.description}
        </p>
        <button
...
const NewOrderPage: React.FC = () => {
  ...
  const categories = [
    "All",
    ...Array.from(new Set(DUMMY_MENU.map((d) => d.category))),
  ];
  ...
  return (
    <div>
      <div className="border-b border-base-300 pb-4 mb-8">
        <h1 className="text-4xl font-serif font-bold text-neutral">
          Create New Order
        </h1>
