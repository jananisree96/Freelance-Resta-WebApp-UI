    ...
    <div className="flex flex-col h-full bg-base-100 border-r border-base-300">
        <div className="flex items-center justify-center h-24 border-b border-base-300">
             <NavLink to="/" className="text-center">
                ...
                <span className="block text-xs uppercase tracking-widest text-neutral-800">Super Admin</span>
            </NavLink>
        </div>
        <nav className="flex-1 p-4 space-y-2">
            <NavItem to="/" icon={<DashboardIcon className="w-5 h-5" />} onClick={onLinkClick}>Dashboard</NavItem>
            <NavItem to="/roles" icon={<RolesIcon className="w-5 h-5" />} onClick={onLinkClick}>Manage Roles</NavItem>
            <NavItem to="/users" icon={<UsersIcon className="w-5 h-5" />} onClick={onLinkClick}>Manage Users</NavItem>
            <NavItem to="/items" icon={<ItemsIcon className="w-5 h-5" />} onClick={onLinkClick}>Manage Items</NavItem>
        </nav>
    </div>
);
            ...
            <div className="flex flex-col flex-1 w-full overflow-hidden">
                ...
                <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
                 ...
                 <footer className="p-4 border-t border-base-300 bg-base-100 text-center text-sm text-neutral-800">
                    &copy; {new Date().getFullYear()} Resta Super Admin Portal. All rights reserved.
                </footer>
            </div>
        </div>
    );
